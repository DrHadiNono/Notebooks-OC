...
def compare_viz(df: pd.DataFrame):
    with st.expander("Comparaison avec les autres clients"):
        modification_container = st.container()
        with modification_container:
...
async def prediction(applicant, nb_features_shap, _explainer, _model):
    ...
    async with aiohttp.ClientSession() as session:
        ...
        with st.expander("Détails", False):
...
async def main(model, explainer, df):
    ...
    with st.sidebar:
        ...
        with st.form(key='settings_form'):
            ...
            nb_features_shap = st.number_input(
                'Nombre de variables (détails)', 1, len(list(df))-1, 20)
            settings_form_submit_button = st.form_submit_button('ok')
    ...
    if settings_form_submit_button or st.session_state.load_prediction:
        ...
        with st.expander("Infos client"):
            ...
            with st.form(key='features_form'):
                ...
                features_form_submit_button = st.form_submit_button('ok')
